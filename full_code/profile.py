...
from rsparse.utils import true_nonzero_params_percentage
...
class LinfLipschitzIndicator:
    ...
    def loss_factory(prox, fgsm):
        ...
        def loss_(lambda_):
            return regularized_cross_entropy(lambda_, reg=reg, fgsm=fgsm)
...
class L1Norm:
    ...
    def loss_factory(prox, fgsm):
        ...
        def loss_(lambda_):
            return regularized_cross_entropy(lambda_, reg, fgsm)
...
def main(
        ...
        dataset, hidden, lambda_, fgsm, cuda,
        ckpt_dir, data_dir, errors_dir, overwrite=False):
    """
    ...
    'dataset', 'hidden', 'lambda_', 'fgsm' are described in the 'help_' object.
    ...
    if os.path.isfile(ckpt) and not overwrite:
        ...
        loss_ = reg_.loss_factory(prox, fgsm)
        torch.manual_seed(seed)
        module.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--reg', type=str, default='l1', help=defaults.help_['reg'])
    parser.add_argument(
            '--epochs', type=int, default=1, help=defaults.help_['epochs'])
    parser.add_argument(
            '--lr', type=float, default=1e-1, help=defaults.help_['lr'])
    parser.add_argument(
            '--prox', action='store_true', default=True,
            help=defaults.help_['prox'])
    parser.add_argument(
            '--dataset', type=str, default='mnist',
            help=defaults.help_['dataset'])
    parser.add_argument(
            '--hidden', type=int, nargs='+', default=[100], help=defaults.help_['hidden'])
    parser.add_argument(
            '--ckpt_dir', type=str, default=defaults.CKPT_DIR,
            help=defaults.help_['ckpt_dir'])
    parser.add_argument(
            '--data_dir', type=str, default=defaults.DATA_DIR,
    ...
    parser.add_argument(
            '--fgsm', type=float, default=0.,
            help=defaults.help_['fgsm'])
    parser.add_argument(
